"""
تيست مرئي: كشف لوحة CV + OCR + ربطها بالسيارة + قياس الأزمنة.
  أخضر  = PROJECT (موقع اللوحة بدون كاشف CV)
  وردي  = CV      (كاشف اللوحة CV اشتغل)
...
from detections import CarDetection, PlateDetector
...
def main():
    ...
    print(f"INIT  car_model={t_car_init*1000:.1f} ms | plate_cv={t_plate_init*1000:.1f} ms")
    ...
    for frame_idx, frame in enumerate(iter_video_frames(INPUT_VIDEO)):
        ...
        loc_state = "CV ON" if s["cv_detect_calls"] > 0 else "CV OFF"
        ocr_state = f"OCR RUN n={len(ocr_ran_ids)}" if ocr_ran_ids else "OCR SKIP"
        banner_bg = (0, 140, 0) if s["cv_detect_calls"] == 0 else (160, 0, 160)
        banner_lines = [
        ...
        cv.putText(
            frame_vis,
            "YELLOW=car | GREEN=PROJECT | MAGENTA=CV | plate text=OCR result",
        ...
        if SHOW_WINDOWS:
            cv.imshow("cars + plates + OCR (CV)", frame_vis)
            key = cv.waitKey(0)
